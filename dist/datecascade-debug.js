define("b5m/datecascade/1.0.0/datecascade-debug", [], function(require, exports, module) {
    var Util = {
        E: function(id) {
    ...
    (function(DateCascade) {
        ...
        DateCascade.onChange = function(domName) {
            var ref = {
                year: "month",
                month: "day"
            }, self = this, refCode = ref[domName] + "Code";
            return function() {
        ...
        DateCascade.loadYear = function() {
            if (!!this.yearDom) {
                ...
                if (options.length <= 1) {
                    ...
                    for (var i = this.curYear; i >= this.curYear - 100; i--) options[options.length] = new Option(i, i);
                }
        ...
        DateCascade.loadMonth = function() {
            if (!!this.monthDom) {
                ...
                //移除所有月选项
                while (options.length) this.monthDom.remove(0);
                options[0] = new Option(" ", 0);
                if (this.yearCode == 0) {
                    this.monthCode = this.monthDom.value;
                    return;
                }
                for (var i = 1; i <= 12; i++) options[options.length] = new Option(i, i);
                //选择默认选项
                for (var i = 0; i < options.length; i++) {
        ...
        DateCascade.loadDay = function() {
            if (!!this.dayDom) {
                ...
                //移除所有天选项
                while (options.length) this.dayDom.remove(0);
                options[0] = new Option(" ", 0);
                if (this.monthCode == 0) return;
                for (var i = 1; i <= this.getDay(this.yearCode, this.monthCode); i++) options[options.length] = new Option(i, i);
                //选择默认选项
                for (var i = 0; i < options.length; i++) {
